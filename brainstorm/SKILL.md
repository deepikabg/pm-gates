...
**Market recon (new-product builds) — run it from HERE, however this gate was entered.** Whether the orchestrator routed here or the user invoked this skill directly: invoke the `market-scan` skill in RECON mode (or, if unavailable, run its recon rules inline — a minutes-long, **sourced** existence check: *does a mature, dominant, or free product already do exactly this?*). This is a sub-check inside Gate 0, not a routing decision — full-scan routing stays with the orchestrator. A hit goes on the table now ("X exists — what's genuinely different about your situation?"), not after the Brief is written. The deep landscape scan deliberately waits until after this interview locks segment + JTBD — scanning the solution category before the problem is framed researches the wrong market.
